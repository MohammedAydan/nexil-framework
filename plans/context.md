...
- Active feature: defineStore-refonte — **COMPLETE (2026-08-31)** — `defineStoreContext` hierarchical StoreContext (StoreContext + Provider/use/create, stableId `nexil:store:${id}`, per-request + global fallback, generic pending, CLI --scoped)
...
- defineStore-refonte: **COMPLETE (2026-08-31)** — `defineStoreContext` hierarchical StoreContext 10/10 (`context-store.test.ts`) + global 41/41 332/332, Vite `--scoped` + `defineStoreContext` regex, client generic pending (no `cart:doubled` hardcode), CLI `nexil g store --scoped` ; see `plans/defineStore-refonte/review.md`.
