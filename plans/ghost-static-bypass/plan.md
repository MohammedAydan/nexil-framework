...
- Mission sequence executed live: clean→build→scaffold→`pnpm dev` → GET / contains "Rendered via Nexis SSR Engine" + `data-nx-on-click="chunk_*.js#handler_*"` + SEO title injected → app build OK → full e2e 9/9 → security clean.
