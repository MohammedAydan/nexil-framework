...
- `RESUMABILITY_BOOTSTRAP` moved to vite-plugin (single source of truth); imports
  chunks via absolute `/nexis-chunks/` URLs.
- vite-plugin dev middleware serves `/nexis-bootstrap.js` + `/nexis-chunks/:name`
...
All 18 public packages bumped 0.1.x/0.1.2 → 2.0.0 to match the v2.0.0 GA tag.
create-nexis-app package remains private/superseded; its NAME survives as a bin
alias on create-nexis (amends ADR-002).
...
- gaapp workspace scaffold: build emits nexis-bootstrap.js + nexis-chunks/chunk_*.js
