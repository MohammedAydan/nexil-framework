# Nexil v1.3.0: تنقل Link وContextScope صريح

> **الحالة: v1.3.0 غير منشور.** إصدار Nexil `v1.2.0` منشور. يصف هذا المستند محتوى الحزم المستهدف بعد هذا tag، ولا يمثل إصدارًا قابلًا للتثبيت حتى يتم tag لالتزام الإصدار وينتهي النشر إلى GitHub Packages.

## تنقل Link دلالي بلا Virtual DOM

يوسع `@nexil/router` واجهة `Link` بعلامة `data-nx-link` دلالية مع الحفاظ على `href` محلي مطلق عادي. يبقى Link anchor عاديًا للـcrawlers والمتصفحات بلا JavaScript. يخرج البناء `nexil-navigation.js` فقط في Routes التي يحتوي HTML الناتج لها على هذه العلامة، ويسجل `BuildRouteRecord.navigationGzipBytes` تكلفته المضغوطة لكل route بقيمة `0` عند عدم وجود Link.
...
`@nexil/starter` محرك بداية portable جديد. تعيد واجهته المكتوبة `createStarterFiles()` سجلات ملفات نسبية فقط، ولا تكتب في نظام الملفات ولا تتصل بالشبكة ولا تثبت packages ولا تنشئ archive ولا تتعامل مع credentials. يضيف مدخل Node تحليل arguments وscaffolding آمنًا بالملفات من أجل المنشئ المنشور وأمر `nexil create`.
...
تستبدل production builds الآن حمولات ScopeRef المسمّاة المضمنة داخل `data-nx-scope` بمفاتيح `nx:scope:<hash>` معتمة. وتخرج أقل حمولة لازمة لاستئناف handlers وSignal-to-DOM bindings مرة واحدة داخل `nexil-state.js` قبل runtime المناسب. Routes الثابتة والصفحات التي لا تحتوي حالة قابلة للاستئناف لا تستلم ملف state.

يقلل ذلك حجم مصدر HTML وظهور أسماء captures و`kind` وstable IDs والقيم الابتدائية داخله. لكنه **لا يجعل البيانات سرية**: يصل `nexil-state.js` إلى المتصفح ويبقى قابلًا للفحص. لا تلتقط credentials أو بيانات طلب خاصة أو أسرار أخرى. يظل client وruntime الخارجيان يقبلان JSON ScopeRef المضمن القديم للتوافق.
...
## ظهور أصول البناء داخل `nexil analyze`

أصبح `nexil analyze` يكمل تقرير JavaScript وCSS لكل Route بجرد للأصول الثابتة الخارجة من `dist/client`. يعرض عدد الملفات غير HTML، وإجمالي حجمها، وحجم الصور، وأكبر خمسة ملفات.
...
يضيف البناء تنبيهًا غير حاجب لصورة حجمها 256 KiB أو أكثر. يقترح التنبيه إنشاء variants مستجيبة من AVIF/WebP، وكتابة `sizes` صحيحة، واستخدام أبعاد أصلية، وتأجيل الصور الواقعة أسفل الجزء المرئي. لا يجعل Nexil التنبيه خطأ build لأن حجم الصورة المناسب يتغير بحسب المحتوى وموضع LCP والـviewport والـCDN.

## خط الصور الثابتة الاختياري

يمكن للتطبيق ضبط `media.images.transform` في `nexil.config.*` لتحويل ملفات PNG وJPEG وSVG العامة أثناء `nexil build`. يحتفظ Nexil بملف الصورة العام الأصلي وينشئ variants ثابتة من AVIF وWebP بالعروض المحددة. تخرج هذه الملفات بجانب مسار المصدر داخل `dist/client`، ويُكتب سجل البناء أيضًا إلى `nexil-media.json`.
...
الـcache الافتراضي هو `.nexil/media-cache`. يمكن حذفه، ويظل داخل جذر المشروع، ويجب تجاهله من source control. يقبل `Image` و`pictureMarkup` خاصية `staticVariants` عندما تريد أن يشير Route إلى الملفات الناتجة بدل عناوين الصور المعتمدة على query. لا تتغير التطبيقات الحالية إلا عندما تفعل الإعداد وتختار markup الثابت صراحة.

## التوافق والتحقق

يبقى إصدار build manifest هو `1`. يظل حقل `assets` اختياريًا، لذلك تظل artifacts الأقدم قابلة للقراءة. `nexil-state.js` مشروط ولا يؤثر في Routes الثابتة. ولأن ناتج production ينسق الآن بين CLI وVite plugin وRouter وruntime الاستئناف، حدّث مجموعة حزم v1.3.0 المتطابقة معًا ولا تثبت CLI قديمًا إلى جانب plugin جديد أو تخلط الملفات المولدة مع build سابق.
