...
يوسع `@nexil/router` واجهة `Link` بعلامة `data-nx-link` دلالية مع الحفاظ على `href` محلي مطلق عادي. يبقى Link anchor عاديًا للـcrawlers والمتصفحات بلا JavaScript. يخرج البناء `nexis-navigation.js` فقط في Routes التي يحتوي HTML الناتج لها على هذه العلامة، ويسجل `BuildRouteRecord.navigationGzipBytes` تكلفته المضغوطة لكل route بقيمة `0` عند عدم وجود Link.
...
```bash
pnpm dlx @nexil/create-nexis@1.3.0 portal --yes --ts --template secure-node
...
يمكن للتطبيق ضبط `media.images.transform` في `nexis.config.*` لتحويل ملفات PNG وJPEG وSVG العامة أثناء `nexis build`. يحتفظ Nexil بملف الصورة العام الأصلي وينشئ variants ثابتة من AVIF وWebP بالعروض المحددة. تخرج هذه الملفات بجانب مسار المصدر داخل `dist/client`، ويُكتب سجل البناء أيضًا إلى `nexis-media.json`.
