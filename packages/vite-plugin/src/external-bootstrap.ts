...
const signal=initial=>{const listeners=new Set,read=()=>initial;Object.defineProperty(read,'value',{enumerable:true,get:()=>initial});read.set=next=>{next=typeof next==='function'?next(initial):next;if(Object.is(initial,next))return;initial=next;for(const listener of[...listeners])listener()};read.subscribe=listener=>(listeners.add(listener),()=>listeners.delete(listener));return read};
...
const scope=element=>{const resolved={};for(const[name,ref]of Object.entries(payload(element))){if(ref.kind==='value')resolved[name]=ref.data;else if(ref.kind==='unsupported')console.warn('[nexis] unsupported scope:',ref.reason);else{let live=registry.get(ref.id);if(!live){if(ref.kind==='signal')live=signal(ref.initial);else if(ref.kind==='store'){const value=signal(ref.initial);live={value,snapshot:()=>value(),set:next=>value.set(next)}}else if(ref.kind==='action')live=input=>fetch(ref.endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(input)}).then(response=>response.json());if(live)registry.set(ref.id,live)}if(live)resolved[name]=live}}return resolved};
