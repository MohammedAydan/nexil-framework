...
import type { Child, ContextScope, ElementNode } from '../core/index.js'
...
// -----------------------------------------------------------------------------
// Layouts, Slot Component & Context Scope Management
// -----------------------------------------------------------------------------

interface RouteContextState {
  readonly match?: RouteMatch
  readonly activeLayoutChild?: Child
}

let activeClientRouteParams: Record<string, string | string[]> = {}
let activeClientLayoutChild: Child | undefined

export function setClientRouteState(
  params: Record<string, string | string[]>,
  child?: Child,
): void {
  activeClientRouteParams = params
  activeClientLayoutChild = child
}

export function getActiveRouteParams<
  T extends Record<string, string | string[]> = Record<string, string | string[]>,
>(): T {
  return (activeClientRouteParams ?? {}) as T
}

export function getActiveLayoutChild(): Child | undefined {
  return activeClientLayoutChild
}

/**
 * Slot component that renders child components in nested layouts.
 */
export function Slot(props?: { readonly children?: Child }): Child {
  const activeChild = getActiveLayoutChild()
  return activeChild ?? props?.children ?? null
}

/**
 * Composes nested layouts wrapping a child page component.
 */
export function composeLayouts(
  layouts: ReadonlyArray<(props: { readonly children: Child }) => Child>,
  page: Child,
): Child {
  return layouts.reduceRight<Child>((child, layout) => layout({ children: child }), page)
}

// -----------------------------------------------------------------------------
// Client-Side SPA Soft Navigation Primitives
// -----------------------------------------------------------------------------
...
export function useSearchParams(): [
  ...
  const setSearchParams = (next: URLSearchParams | Record<string, string>) => {
    ...
    navigate(`${location.pathname}?${nextParams.toString()}${location.hash}`)
  }
