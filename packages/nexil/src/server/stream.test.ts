...
import { renderToAsyncIterable, renderToStream } from './stream.js'
...
describe('renderToStream', () => {
  ...
  it('produces byte-identical output to buffered rendering', async () => {
    ...
    await expect(readText(renderToStream(root, { chunkSize: 5 }))).resolves.toBe(
      renderToString(root),
    )
  })
  ...
  it('supports renderToAsyncIterable with ContextScope isolation', async () => {
    const RequestIdContext = createContext<string>('default')
    const scope = provideContext(createContextScope(), RequestIdContext, 'req-42')

    const tree = element('div', { id: 'ctx-test' }, () => RequestIdContext.use())
    const chunks: string[] = []
    for await (const chunk of renderToAsyncIterable(tree, scope)) {
      chunks.push(chunk)
    }
    expect(chunks.join('')).toBe('<div id="ctx-test">req-42</div>')
  })
})
