  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--yes' || arg === '-y') yes = true
      ...
      if (
        ...
        throw new Error('Template must be minimal, interactive, secure-node, blank, or fullstack.')
      template = value as ScaffoldTemplate
...
async function promptOptions(options: ScaffoldOptions): Promise<ResolvedScaffoldOptions> {
  ...
  try {
    ...
    const template =
      ...
      ((
        await readline.question(
          'Template: fullstack, blank, interactive, minimal, or secure-node? [fullstack] ',
        )
      )
        .trim()
        .toLowerCase() ||
    ...
    if (
      ...
      throw new Error('Template must be fullstack, blank, minimal, interactive, or secure-node.')
...
async function findFrameworkRoot(start: string): Promise<string | undefined> {
  ...
  while (true) {
    try {
      ...
      await access(join(current, 'packages', 'nexil', 'package.json'))
      return current
    } catch {
...
export async function scaffoldProject(
  ...
  const files = createStarterFiles({
    ...
    dependencyVersion: frameworkRoot ? 'workspace:*' : '^0.2.0',
  })
