...
const nexOrigin = process.env.nexil_COMPARE_ORIGIN ?? 'http://127.0.0.1:4173'
...
async function routeJavaScriptBytes(directory) {
  ...
  for (const chunk of html.matchAll(/data-nx-on-[^=]+="([^"]*chunk_[a-f0-9]{12}\.js)/g))
    files.add(`/nexil-chunks/${chunk[1]}`)
...
async function measure(browser, origin, path = '/') {
  ...
  await page.addInitScript(() => {
    window.__nexilVitals = { lcp: null, cls: 0, inp: null }
    new PerformanceObserver((list) => {
      const entries = list.getEntries()
      const last = entries.at(-1)
      if (last) window.__nexilVitals.lcp = last.startTime
    }).observe({ type: 'largest-contentful-paint', buffered: true })
    new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (!entry.hadRecentInput) window.__nexilVitals.cls += entry.value
      }
    ...
    try {
      new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          ...
          window.__nexilVitals.inp = Math.max(window.__nexilVitals.inp ?? 0, duration)
        }
...
try {
  const [nexil, astro] = await Promise.all([
    measure(browser, nexOrigin),
    measure(browser, baselineOrigin),
  ])
  const nexilBundle = await routeJavaScriptBytes(join(showcaseRoot, 'dist', 'client'))
  const astroBundle = await routeJavaScriptBytes(baselineRoot)
  const clientJsBytes = nexilBundle.bytes
