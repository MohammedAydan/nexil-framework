...
const server =
  process.env.BENCH_USE_EXISTING === '1'
    ...
    : spawn(
        ...
        {
          cwd: root,
          env: { ...process.env, NEXIL_HOST: '127.0.0.1', NEXIL_PORT: String(port) },
          stdio: ['ignore', 'pipe', 'pipe'],
        },
      )
try {
  if (server) await waitForServer(server)
  const manifest = JSON.parse(await readFile(join(dist, 'nexil-manifest.json'), 'utf8'))
  const measured = []
  ...
  const actionResponse = await fetch(`${base}/__nexil/actions/labs/submit`, {
    method: 'POST',
    headers: {
  ...
  const bootstrap = await fileMetrics(join(dist, 'nexil-bootstrap.js'))
  const chunkNames = await readdir(join(dist, 'nexil-chunks'))
  const chunks = []
  for (const chunkName of chunkNames.filter((name) => name.endsWith('.js')).sort())
    chunks.push({ name: chunkName, ...(await fileMetrics(join(dist, 'nexil-chunks', chunkName))) })
  const assets = []
