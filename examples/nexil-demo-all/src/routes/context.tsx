...
export const seo = { title: 'Context & StoreContext — Nexil Demo', description: 'Hierarchical DI: createContext vs defineStoreContext.' }
  ...
  return (
    <div class="space-y-8">
      <header>
        ...
        <p class="mt-2 text-slate-400">React-like `createContext` (value) + Nexil `defineStoreContext` (store).</p>
      </header>
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          {(() => {
            const outer = SimpleCtx.Provider({
              ...
              children: () => {
                ...
                const inner = SimpleCtx.Provider({ value: 'inner', children: () => SimpleCtx.use() })
        ...
        <p class="mt-2 text-xs text-slate-500">Rendered at SSR time via `Provider` nesting — no JS needed.</p>
      </section>

      {/* StoreContext */}
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <h2 class="font-bold text-white">defineStoreContext · Store + Context</h2>
        <p class="mt-1 text-sm text-slate-400">`src/stores/theme.ts` — stableId `nexil:store:theme`.</p>
        <p class="mt-3 text-sm text-slate-300">
          Fallback mode: <b class={fallback.mode === 'dark' ? 'text-violet-400' : 'text-amber-400'}>{fallback.mode}</b> · accent {fallback.accent}
        </p>
        <div class="mt-3 flex flex-wrap gap-2">
          <button class="rounded bg-white px-3 py-1 text-xs font-bold text-slate-900" onClick$={() => fallback.toggle()}>
            toggle fallback
          </button>
          <button
          ...
          {(() => {
            ...
            const demo = ThemeStore.Provider({
              ...
              children: () => {
                ...
                const innerRes = ThemeStore.Provider({ value: innerStore, children: () => ThemeStore.use().mode })
        ...
        <p class="mt-2 text-xs text-slate-500">`create()` gives a fresh isolated instance per Provider — perfect for per-layout theme.</p>
      </section>
...
ThemeStore.use(next) // 'dark' — testable without ALS`}
        ...
        <p class="mt-2 text-xs text-slate-500">
          For Cloudflare Workers: wrap `fetch` with `runWithScope(ctx.scope, ...)` — see `src/stores/theme.ts`.
        </p>
      </section>
    </div>
  )
