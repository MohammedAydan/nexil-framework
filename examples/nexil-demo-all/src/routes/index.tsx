...
export const seo = {
  ...
  description:
    'Comprehensive demo of Nexil framework: signals, stores, StoreContext, context, resumability, routing, forms, media, and more.',
}
    ...
    <div class="space-y-12">
      ...
      <section class="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-800 p-8 shadow-2xl">
        ...
        <p class="mt-4 max-w-2xl text-slate-300">
          This demo implements <strong>every</strong> Nexil feature practically: signals, stores
          (unified/modular/context), context, resumability, routing, forms, media, SEO, and
          zero-hydration. Inspect the HTML — no JS before you click.
        </p>
        <div class="mt-6 flex flex-wrap gap-3">
          <Link
            href="/stores"
            class="rounded-xl bg-cyan-400 px-5 py-3 text-sm font-bold text-slate-950 hover:bg-cyan-300"
          >
            Explore Stores →
          </Link>
          <Link
            href="/shop"
            class="rounded-xl border border-slate-700 px-5 py-3 text-sm font-bold text-slate-200 hover:border-cyan-400"
          >
            Go to Shop
          </Link>
          <span class="rounded-xl bg-slate-800 px-4 py-3 text-sm text-slate-400">
            Build: <code>pnpm build && pnpm start</code>
          </span>
        </div>
      </section>
      ...
      <section class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            O(1) DOM updates, no VDOM. Try the buttons — only the text node changes.
          </p>
          <div class="mt-4 flex items-center gap-4">
            ...
            <span class="text-sm text-slate-300">
              doubled ={' '}
              <strong bindText$={doubledLocal} class="text-cyan-400">
                {String(doubledLocal())}
              </strong>
            </span>
            <button
          ...
          <p class="mt-3 text-xs text-slate-500">
            theme: {theme()} · try batch to coalesce two sets into one flush
          </p>
        </div>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            Singleton `src/stores/counter.ts` with `state`, `getters`, `actions`.
          </p>
          <div class="mt-4 flex flex-wrap items-center gap-3">
            <span class="text-sm text-slate-300">
              count:{' '}
              <strong class="text-white" bindText$={counter.count}>
                {String(counter.count)}
              </strong>{' '}
              · doubled:{' '}
              <strong class="text-cyan-400" bindText$={counter.doubled}>
                {String(counter.doubled)}
              </strong>
            </span>
            <button
              class="rounded-lg bg-cyan-500 px-3 py-1.5 text-xs font-bold text-slate-950"
              onClick$={() => counter.inc()}
            >
              inc
            </button>
            <button
              class="rounded-lg bg-slate-800 px-3 py-1.5 text-xs text-white"
              onClick$={() => counter.dec()}
            >
              dec
            </button>
          ...
          <p class="mt-2 text-xs text-slate-500">
            Survives Link navigation via `__NEXIL_STORES__` · isEven: {String(counter.isEven)}
          </p>
        </div>
      </section>
      ...
      <section class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            `src/stores/user/` split: `types.ts` + `actions.ts` + `store.ts`
          </p>
          <div class="mt-3 space-y-2 text-sm">
            <p class="text-slate-300">
              User: <strong class="text-white">{user.profile?.name ?? '—'}</strong> (
              {user.profile?.email}) · count:{' '}
            ...
            <div class="flex gap-2">
              ...
              <button
                ...
                onClick$={() =>
                  user.setProfile({ name: 'Eve', email: 'eve@nexil.dev', role: 'user' })
                }
              >
                set Eve
              </button>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            `push`/`splice` are proxied → structural sharing + O(1) DOM.
          </p>
          <p class="mt-2 text-sm text-slate-300">
            Items:{' '}
            <span class="font-bold text-white" bindText$={cart.totalItems}>
              {String(cart.totalItems)}
            </span>{' '}
            · Total: $
            <span class="font-bold text-cyan-400" bindText$={cart.totalPrice}>
      ...
      <section class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            Hierarchical like React `createContext` — nearest Provider wins.
          </p>
          <p class="mt-3 text-sm text-slate-300">
            Mode:{' '}
            <strong class={isDark() ? 'text-violet-400' : 'text-amber-400'}>{themeCtx.mode}</strong>{' '}
            · accent: {themeCtx.accent}
          </p>
          <div class="mt-3 flex gap-2">
            ...
            <button
              class="rounded bg-slate-800 px-3 py-1 text-xs text-white"
              onClick$={() => ThemeStore.create({ mode: 'dark' })}
            >
          ...
          <p class="mt-2 text-xs text-slate-500">
            Try{' '}
            <Link href="/context" class="underline">
              /context
            </Link>{' '}
            for nested Providers demo.
          </p>
        </div>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p class="mt-1 text-sm text-slate-400">
            No JS until first interaction. Check `data-nx-scope` in HTML.
          </p>
          <button
          ...
          <p class="mt-2 text-xs text-slate-500">
            This handler is a lazy chunk `/nexil-chunks/chunk_*.js` loaded on demand.
          </p>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        <p class="mt-4 text-xs text-slate-500">
          Dynamic route `shop/[id].tsx` uses `routeLoader$` for per-request data. Try{' '}
          <Link href="/shop/p2" class="underline">
            /shop/p2
          </Link>
          .
        </p>
      ...
      <section class="flex flex-wrap gap-2">
        {[
          ...
          <Link
            key={l.href}
            href={l.href}
            class="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white"
          >
            {l.label}
          </Link>
        ))}
