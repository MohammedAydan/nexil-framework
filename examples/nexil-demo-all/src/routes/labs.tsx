...
export const seo = {
  title: 'Labs — Nexil Demo',
  description: 'Resource, effect, watch, batch, and resumability labs.',
}
...
export default component(() => {
  ...
  effect(() => {
    ...
    logs.set((prev) => [
      ...prev.slice(-4),
      `effect: count=${c} at ${new Date().toLocaleTimeString()}`,
    ])
  })
    ...
    <div class="space-y-8">
      <header>
        ...
        <p class="mt-2 text-slate-400">
          Playground for `resource`, `effect`, `batch`, `computed` cycles, and `For`/`Show`.
        </p>
      </header>
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        <p class="mt-1 text-sm text-slate-400">
          `resource(() =&gt; fetch(...))` with `loading`/`error`/`refetch`.
        </p>
        <div class="mt-3 text-sm">
          ...
          <Show
            when={user.error()}
            fallback={<p class="text-slate-300">User: {user()?.name ?? '—'}</p>}
          >
            <p class="text-red-400">Error: {String(user.error()?.message)}</p>
          </Show>
        </div>
        <button
          class="mt-3 rounded bg-white px-3 py-1 text-xs font-bold text-slate-900"
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        <div class="mt-3 space-y-2 text-sm">
          <For each={['Alpha', 'Beta', 'Gamma']}>
            {(item: string) => (
              <div class="rounded bg-slate-800 px-3 py-2 text-slate-200">{item}</div>
            )}
          </For>
          <Show
            when={count() > 3}
            fallback={<p class="text-slate-500">Count ≤ 3 — keep clicking</p>}
          >
      ...
      <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        <p class="mt-1 text-sm text-slate-400">
          Every button above is `onClick$` — inspect HTML for `data-nx-scope` and lazy chunks.
        </p>
