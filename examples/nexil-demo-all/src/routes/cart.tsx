...
export const seo = { title: 'Cart — Nexil Demo', description: 'Cart store with fine-grained bindings and Link persistence.' }
      ...
      <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div class="flex items-center justify-between">
          <p class="text-sm text-slate-300">
            Items: <b class="text-white" bindText$={cart.totalItems}>{String(cart.totalItems)}</b> · Total: $
            <b class="text-cyan-400" bindText$={cart.totalPrice}>{String(cart.totalPrice)}</b>
          </p>
          <button class="rounded-lg bg-slate-800 px-3 py-1 text-xs text-white hover:bg-slate-700" onClick$={() => cart.clear()}>
            Clear
          </button>
        </div>
        ...
        <div class="mt-4 space-y-2">
            ...
            {(item: any) => (
              <div class="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-800 p-3">
                <div>
                  ...
                  <p class="text-xs text-slate-400">${item.price} × {item.quantity}</p>
                </div>
                <div class="flex items-center gap-2">
                  <button class="rounded bg-slate-700 px-2 py-1 text-xs text-white" onClick$={() => cart.decQty(item.id)}>
                    −
                  </button>
                  <span class="w-6 text-center text-sm text-white">{item.quantity}</span>
                  <button class="rounded bg-cyan-500 px-2 py-1 text-xs font-bold text-slate-950" onClick$={() => cart.incQty(item.id)}>
                    +
                  </button>
                  <button class="rounded bg-red-900/50 px-2 py-1 text-xs text-red-300" onClick$={() => cart.removeItem(item.id)}>
                    ✕
                  </button>
                </div>
              </div>
          ...
          {cart.items.length === 0 && (
            <p class="rounded-xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-500">
              Cart is empty. <Link href="/shop" class="text-cyan-400 underline">Add items from shop</Link>.
            </p>
          )}
        </div>

        <div class="mt-6 flex gap-3">
          <Link href="/shop" class="flex-1 rounded-xl bg-cyan-500 py-3 text-center text-sm font-bold text-slate-950 hover:bg-cyan-400">
            Continue shopping
          </Link>
          <button
...
data-nx-store-bind="cart:totalItems#text"`}
        ...
        <p class="mt-2 text-xs text-slate-500">Only accessed stores are serialized. `&lt;` escaped as `\u003c` — XSS safe.</p>
      </section>
    </div>
  )
})
