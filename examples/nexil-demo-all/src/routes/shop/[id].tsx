...
export const seo = { title: 'Product — Nexil Demo', description: 'Dynamic route with routeLoader$.' }
...
export const useProduct = routeLoader$(async (event) => {
  ...
  const product = catalog[id] ?? { name: `Item #${id}`, price: 9, desc: 'Unknown item — still resumable' }
  return { id, ...product }
})

export default component(({ data }: { data?: { id: string; name: string; price: number; desc: string } }) => {
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <p class="text-xs font-bold uppercase tracking-[0.3em] text-cyan-400">PRODUCT #{data.id}</p>
        ...
        <div class="mt-6 flex gap-3">
          <button class="rounded-xl bg-cyan-500 px-5 py-3 text-sm font-bold text-slate-950 hover:bg-cyan-400" onClick$={() => cart.addItem({ id: data.id, name: data.name, price: data.price })}>
            Add to cart — $<span bindText$={cart.totalPrice}>{String(cart.totalPrice)}</span>
          </button>
          <Link href="/cart" class="rounded-xl border border-slate-700 px-5 py-3 text-sm font-bold text-slate-200">
            View cart ({String(cart.totalItems)})
          </Link>
        </div>
        <p class="mt-4 text-xs text-slate-500">
          Loader: `routeLoader$(async (event) =&gt; event.params.id)` — per-request, serializable, SSR streaming.
        </p>
      </div>
    </div>
  )
